...
@Composable
fun MapScreen4(navController: NavController){
    ...
    val cameraPositionState = rememberCameraPositionState { position =
        CameraPosition.fromLatLngZoom(LatLng(40.650344886136885,  -74.28560496735331), 5f) }
    GoogleMap(
        modifier = Modifier
            .fillMaxSize()
    ...
    Column {
        Map4TypeControls(onMapTypeClick = {
...
@Composable
fun Map4TypeButton(type: MapType, onClick: () -> Unit) {
    MapButton(text = type.toString(), onClick = onClick)
}

@Composable
fun Map4TypeControls(
    ...
    Row(
        ...
        MapType.values().forEach {
            Map4TypeButton(type = it) { onMapTypeClick(it) }
        }
    }
}
@Composable
