...
@Composable
fun MapScreen3(navController: NavController){
    ...
    val cameraPositionState = rememberCameraPositionState { position =
        CameraPosition.fromLatLngZoom(LatLng(27.175688037153474,  78.04213849823365), 5f) }
    GoogleMap(
        modifier = Modifier
            .fillMaxSize()
    ...
    Column {
        Map3TypeControls(onMapTypeClick = {
...
@Composable
fun Map3TypeButton(type: MapType, onClick: () -> Unit) {
    MapButton(text = type.toString(), onClick = onClick)
}

@Composable
fun Map3TypeControls(
    ...
    Row(
        ...
        MapType.values().forEach {
            Map3TypeButton(type = it) { onMapTypeClick(it) }
        }
    }
}
@Composable
